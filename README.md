...
- 管理员账号：`admin`
- 首次启动前必须通过 `ADMIN_PASSWORD` 设置管理员密码；未设置或不符合密码策略时，应用会停止启动，避免创建默认弱密码账号。
- 默认角色：`ADMIN`
- 默认项目：`默认项目`
- 默认项目编码：`DEFAULT`
...
- 导入不会自动更新已有资产；同一项目下资产名称重复、内网 IP 重复、外网 IP 重复，以及同一文件内的重复行都会被拒绝，并记录到失败行明细。
...
`liveness` 和 `readiness` 健康探针允许匿名访问，便于裸机、Docker 和 Kubernetes 探活；`metrics` 和 `prometheus` 需要携带管理员令牌访问。生产环境建议仍在网关层限制监控接口来源，并为 Prometheus 抓取配置专用的管理员令牌或通过内网网关注入认证头。
